...
def production_fails_closed(root: Path) -> tuple[bool, str]:
...
def main() -> int:
    ...
    print("[RELEASE17] check-start name=production fails closed", flush=True)
    ok, detail = production_fails_closed(root)
    results.append(GateResult("production fails closed", "passed" if ok else "blocked", detail))
    if ok:
        print("[RELEASE17] production-fails-closed-ok")
