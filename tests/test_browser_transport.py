...
class BrowserTransportTests(unittest.TestCase):
    ...
    def test_rest_schemas_and_versioned_resources(self) -> None:
        ...
        self.assertEqual(
            f"/api/v1/sessions/{response.session_id}",
            RestResourceCatalog.session(response.session_id),
        )
    ...
    def test_ordering_and_session_correlation(self) -> None:
        ...
        first = channel.publish(
            SessionEventType.SESSION_STARTED, {}, "bootstrap", "event-1"
        )
        duplicate = channel.publish(
            SessionEventType.SESSION_STARTED, {}, "bootstrap", "event-1"
        )
        second = channel.publish(
            SessionEventType.SESSION_ACTIVE, {}, "bootstrap", "event-2"
        )
        self.assertIs(first, duplicate)
        self.assertEqual(
            [1, 2], [event.sequence for event in channel.replay(session_id)]
        )
    ...
    def test_signaling_and_media_control_separation(self) -> None:
        ...
        offer = SignalingMessage(
            session_id, SignalingType.OFFER, {"sdp": "v=0"}, "negotiation-1"
        )
    ...
    def test_signaling_answer_and_ice_candidate_validation(self) -> None:
        ...
        answer = SignalingMessage(
            session_id, SignalingType.ANSWER, {"sdp": "v=0"}, "negotiation-1"
        )
        self.assertEqual(SignalingType.ANSWER, answer.message_type)
        with self.assertRaises(TransportValidationError):
            SignalingMessage(
                session_id, SignalingType.ANSWER, {"sdp": ""}, "negotiation-1"
            )
        with self.assertRaises(TransportValidationError):
        ...
        with self.assertRaises(TransportValidationError):
            SignalingMessage(
                session_id, SignalingType.ICE_CANDIDATE, {}, "negotiation-2"
            )
    ...
    def test_signaling_payload_size_limit(self) -> None:
        ...
        msg = SignalingMessage(
            session_id, SignalingType.OFFER, valid_payload, "negotiation-1"
        )
        self.assertIsNotNone(msg)
        oversized_sdp = "x" * (MAX_SIGNALING_PAYLOAD_BYTES + 1)
        with self.assertRaises(TransportValidationError) as ctx:
            SignalingMessage(
                session_id, SignalingType.OFFER, {"sdp": oversized_sdp}, "negotiation-1"
            )
