...
export function createSessionTransport({ WebSocketImpl = globalThis.WebSocket, url, token: _token, sessionId, maxReconnects = 3, onEvent = () => {} }) {
  ...
  const connect = () => {
    if (!WebSocketImpl) { setState(CONNECTION_STATES.FAILED); markBrowser('WEBSOCKET_FAILED'); return }
    ...
    const reconnecting = Boolean(attempts)
    setState(reconnecting ? CONNECTION_STATES.RECONNECTING : CONNECTION_STATES.CONNECTING)
    markBrowser(reconnecting ? 'WEBSOCKET_RECONNECTING' : 'WEBSOCKET_CONNECTING')
    socket = new WebSocketImpl(url)
    socket.onopen = () => { attempts = 0; setState(CONNECTION_STATES.CONNECTED); markBrowser('WEBSOCKET_CONNECTED'); send('connect', {}, `connect-${Date.now()}`, cursor) }
    socket.onmessage = message => {
      const event = JSON.parse(message.data)
      if (event.sequence && event.sequence > cursor) cursor = event.sequence
      eventHandler(event)
    }
    socket.onclose = () => { if (intentionalDisconnect) { markBrowser('WEBSOCKET_DISCONNECTED'); return } if (attempts < maxReconnects) { attempts += 1; setState(CONNECTION_STATES.RECONNECTING); markBrowser('WEBSOCKET_RECONNECTING') } else { setState(CONNECTION_STATES.FAILED); markBrowser('WEBSOCKET_FAILED') } }
    socket.onerror = () => { if (!intentionalDisconnect) { setState(CONNECTION_STATES.RECONNECTING); markBrowser('WEBSOCKET_RECONNECTING') } }
  }
  ...
  const disconnect = () => { intentionalDisconnect = true; if (socket?.readyState === 1) send('close'); socket?.close(); setState(CONNECTION_STATES.DISCONNECTED); markBrowser('WEBSOCKET_DISCONNECTED') }
