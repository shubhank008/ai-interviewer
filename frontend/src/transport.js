...
export function createSessionTransport({ WebSocketImpl = globalThis.WebSocket, url, token, sessionId, maxReconnects = 3, onEvent = () => {} }) {
  ...
  const connect = () => {
    ...
    socket = new WebSocketImpl(`${url}?token=${encodeURIComponent(token)}`)
    ...
    socket.onclose = () => { if (attempts < maxReconnects) { attempts += 1; setState(CONNECTION_STATES.RECONNECTING) } else setState(CONNECTION_STATES.FAILED) }
    socket.onerror = () => setState(CONNECTION_STATES.RECONNECTING)
  }
  ...
  const disconnect = () => { if (socket?.readyState === 1) send('close'); socket?.close(); setState(CONNECTION_STATES.DISCONNECTED) }
