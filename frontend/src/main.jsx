...
export function App({ authProvider = auth, fetcher = fetch }) {
  ...
  const transport = useMemo(() => session ? createSessionTransport({ url: sessionWebSocketUrl(session.id, token), token, sessionId: session.id }) : null, [session])
  const media = useMemo(() => session ? createMediaTransport() : null, [session])
  ...
  return <Shell user={user} activeView={view} onView={navigate} onSignOut={async () => { try { await authProvider.signOut() } catch { /* signOut errors are non-blocking */ } setUser(null) }}>{offline && <div className="offline-bar">You are offline. Local controls remain available; synced resources may be out of date.</div>}{view === 'setup' && <SetupView api={client} token={token} onStarted={data => { setSession(data); setView('active') }} />}{view === 'active' && <ActiveView session={session} transport={transport} media={media} onComplete={() => setView('results')} />}{view === 'history' && <HistoryView history={history} loading={loading} onOpen={() => setView('results')} onDelete={deleteInterview} />}{view === 'results' && <ResultsView />}</Shell>
}
