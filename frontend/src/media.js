export function createMediaTransport({ navigatorImpl = globalThis.navigator, peerConnectionFactory = globalThis.RTCPeerConnection, onState = () => {} } = {}) {
  ...
  async function requestMicrophone() {
    if (!navigatorImpl?.mediaDevices?.getUserMedia) { setState('denied'); throw new Error('microphone unavailable') }
    try { stream = await navigatorImpl.mediaDevices.getUserMedia({ audio: true }); setState('ready'); return stream } catch (error) { setState('denied'); throw error }
  }
  ...
  function teardown() { stream?.getTracks().forEach(track => track.stop()); peer?.close(); stream = null; peer = null; setState('closed') }
