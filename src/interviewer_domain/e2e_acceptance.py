...
class AcceptanceReport:
    ...
    def fail(self, name: str, error: str) -> None:
        ...
        self.events.append(EvidenceEvent(name, "failed", {"error": _redact_text(error)}))
...
def configured_integration_report(environ: dict[str, str] | None = None) -> dict[str, dict[str, Any]]:
    ...
    for name, keys in requirements.items():
        ...
        report[name] = {
            ...
            "reason": "configuration present" if configured else "required configuration absent",
...
class CandidateSimulation:
    ...
    def __init__(self) -> None:
        ...
        self.llm = InMemoryStreamingLLM("I led a reliable API migration and measured impact.")
    ...
    async def audio(self, prompt: str) -> bytes:
        ...
        chunks = [chunk async for chunk in self.llm.generate_stream(prompt, CancellationToken())]
        text = "".join(chunk.text for chunk in chunks)
        audio = [chunk async for chunk in self.tts.synthesize_stream(text, CancellationToken())]
        return b"".join(chunk.audio for chunk in audio)


async def run_local_acceptance(output_directory: str | Path = ".agent_tmp/phase14-evidence") -> AcceptanceReport:
    ...
    try:
        ...
        with api_client.websocket_connect(f"/ws/v1/sessions/{api_session_id}?token=dev-token") as socket:
            socket.send_json({"session_id": api_session_id, "type": "connect", "correlation_id": "phase14-connect", "payload": {}})
            connected = socket.receive_json()
            if connected["type"] != "connection":
                raise AssertionError("browser WebSocket did not connect")
            socket.send_json({"session_id": api_session_id, "type": "heartbeat", "correlation_id": "phase14-heartbeat", "payload": {}})
        ...
        report.record("auth-setup", outcome="authenticated", mode=record.mode.value, api_session=api_session_id)
        ...
        if not evidence or not any(item.chunk.source.value == "resume" for item in evidence):
            raise AssertionError("resume evidence was not retrieved")
        report.record("pdf-rag", chunks=len(job_chunks + resume_chunks), evidence=len(evidence))

        candidate = CandidateSimulation()
        interviewer_llm = InMemoryStreamingLLM("How did you validate the migration and its impact?")
        ...
        report.record("providers-separated", candidate_llm="in-memory-candidate", interviewer_llm="in-memory-interviewer")

        browser = BrowserSessionState(record.id, user_id)
        media = LocalWebRTCBoundary(record.id)
        offer = SignalingMessage(record.id, SignalingType.OFFER, {"sdp": "v=0 local"}, "offer-1")
        ...
        report.record("browser-media-loop", frame_bytes=len(received_audio), signaling=media.connected)

        events = InMemoryEventBus()
        live_store = InMemoryDataStore()
        interviewer = LiveVoiceOrchestrator(
            InterviewSession(user_id, record.mode, record.id),
            [InMemoryStreamingSTT("I led a reliable API migration and measured impact.")],
            [interviewer_llm],
            [interviewer_tts],
            live_store,
            events,
        ...
        first = await interviewer.process_turn(Turn(record.id, 1, "candidate", received_audio))
        ...
        second = await interviewer.process_turn(Turn(record.id, 2, "candidate", await media.playback()))
        ...
        report.record("interruption-reconnect", replay=len(replay), resumed=len(resumed), interrupted=browser.interrupted)
        ...
        report.record("replay-deletion", history_count=len(history), result_available=True, deleted=True)
...
def run_local_acceptance_sync(output_directory: str | Path = ".agent_tmp/phase14-evidence") -> AcceptanceReport:
