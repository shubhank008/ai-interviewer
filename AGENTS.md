...
* Phase 16 rehearsal fixtures are `tests/demo_resume.pdf` and `tests/demo_jobdescription.txt`. Missing fixtures are a failed precondition, not a reason to silently skip live evidence. Scanned or encrypted PDFs fail gracefully because the Phase 16 parser is text-only.
