...
* Phase 16 uses Firebase email/password and Google Sign-In with open signup, Firebase Admin verification, EU Firestore, Firebase Storage with local/NFS checks; FTP is deferred, WhisperX small on CPU, Kokoro English with randomized per-call voices, OpenRouter through `LLM_API_KEY` and the configured `LLM_MODEL` with a 60-second timeout and three transient retries, and one Docker container initially benchmarked on self-hosted/Railway.
...
* Phase 16 browser audio starts with timestamped microphone chunks and the selected VAD/streaming-Whisper seam; custom WebRTC/TURN is not a prerequisite unless benchmark evidence selects it. S3 is out of scope; `STORAGE_BACKEND=gcs` is the runtime selector for the locked Firebase Storage adapter, while `local` is reserved for development and deterministic checks.
* The Phase 16 rehearsal runner must execute configured live providers and both agent-to-agent journeys, report skipped configuration versus failed selected providers, and emit only redacted evidence and benchmark metadata. A configured provider that was not exercised is a failure, not a skip.
...
* Phase 16 locked policy: live rehearsal is programmatic agent-to-agent E2E, with an Interviewer Agent and Interviewee Agent exchanging timestamped audio buffers. Browser URL, browser automation, WebRTC, and TURN are deferred from Phase 16 acceptance.
* Phase 16 locked providers are Firebase Web Auth email/password plus Google Sign-In, EU Firestore, Firebase Storage bucket `gs://the-interviewer-c3a01.firebasestorage.app` plus local/NFS checks, WhisperX small/small.en on CPU, Kokoro 0.9.4 English at 24 kHz mono PCM, and OpenRouter with `LLM_API_KEY`; FTP is deferred.
* Phase 16 rehearsal fixtures are `tests/demo_resume.pdf` and `tests/demo_jobdescription.txt`. Missing fixtures are a failed precondition, not a reason to silently skip live evidence. The rehearsal may use a UUID-only internal owner fixture; Firebase JWT verification and owner-isolation testing are explicitly deferred to a future authenticated E2E task and must never be accepted by production. Scanned or encrypted PDFs fail gracefully because the Phase 16 parser is text-only.
