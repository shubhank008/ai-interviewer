...
* Phase 16 is live-first: offline tests may protect contracts but cannot satisfy release acceptance. The beta provider decisions are passwordless Firebase email-link Auth with open signup, EU Firestore, local/NFS-compatible plus FTP and Firebase Storage, WhisperX small, Kokoro English with randomized per-call voices, OpenRouter `~deepseek/deepseek-v4-flash-latest` with 60-second timeout and three transient retries, and a single Docker container initially benchmarked on self-hosted/Railway.
...
* Phase 16 browser audio must begin with the simplest reliable timestamped chunk/VAD/streaming-Whisper approach; Silero VAD, SimulStreaming, and full WebRTC/TURN are evaluation options rather than assumed requirements. S3/GCS and the separate admin panel are out of scope for this phase.
* The Phase 16 rehearsal runner must exercise configured live providers and browser journeys, report skipped configuration versus failed selected providers, and emit only redacted evidence and benchmark metadata.
